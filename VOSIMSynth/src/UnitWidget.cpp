...
void synui::UnitWidget::draw(NVGcontext* ctx)
{
    ...
    nanogui::Color oColor = nanogui::Color(0.1f, 0.07f, 0.0f, 0.3f);
    nanogui::Color oColor2 = Vector4f(1, 1, 1, 0.667).cwiseProduct(oColor);
    nanogui::Color iColor = nanogui::Color(0.0f, 0.3f);
    ...
    if (m_state == TitleClicked)
    {
        ...
        Vector2i mousePos = static_cast<nanogui::Screen*>(screen)->mousePos() - absolutePosition();
        double distanceCovered = (mousePos - m_clickPos).squaredNorm();
        if (distanceCovered > 1.0) { m_state = TitleDragging; }
    }
...
Eigen::Vector2i synui::UnitWidget::getInputPortAbsPosition(int a_portId)
{
    ...
    return port->absolutePosition() + Vector2f{ 0, m_parentCircuit->gridSpacing() * 0.5 }.cast<int>();
}

Eigen::Vector2i synui::UnitWidget::getOutputPortAbsPosition(int a_portId)
{
    auto& port = m_outputLabels[a_portId];
    return port->absolutePosition() + Vector2f{ port->width(), m_parentCircuit->gridSpacing() * 0.5 }.cast<int>();
}
...
void synui::UnitWidget::updateRowSizes_()
{
    ...
    while (rowHeight < 10) { rowHeight += m_parentCircuit->gridSpacing(); }

    // Special sizing for title label (so grid point will be centered relative to port labels)
    int titleRowSize = rowHeight + m_parentCircuit->gridSpacing() * 0.5;
