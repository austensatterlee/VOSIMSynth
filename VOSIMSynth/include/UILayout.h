...
/**
 *  \file UILayout.h
 *  \brief Adapted from NanoGUI's Layout class
 *  \details See https://github.com/wjakob/nanogui
...
namespace synui
{
	...
	public:
		UILayout() {}
		virtual ~UILayout() { }
		virtual void performLayout(NVGcontext *ctx, UIComponent *widget) const = 0;
		virtual Vector2i preferredSize(NVGcontext *ctx, const UIComponent *widget) const = 0;
	};
	...
	* widgets
	*/
	class BoxLayout : public UILayout {
	public:
		/**
		* \brief Construct a box layout which packs widgets in the given \c orientation
		* \param alignment
		*    UIComponent alignment perpendicular to the chosen orientation.
		* \param margin
		...
		*/
		...
		virtual Vector2i preferredSize(NVGcontext *ctx, const UIComponent *widget) const override;
		virtual void performLayout(NVGcontext *ctx, UIComponent *widget) const override;
	...
	*/
	class  GroupLayout : public UILayout {
	public:
	...
	* \brief Grid layout
	*
	* UIComponents are arranged in a grid that has a fixed grid resolution \c resolution
	...
	*/
	...
	protected:
		...
		void computeLayout(NVGcontext *ctx, const UIComponent *widget,
	...
	*/
	class  AdvancedGridLayout : public UILayout {
	public:
		struct Anchor {
			uint8_t pos[2];
			uint8_t size[2];
		...
		void setAnchor(const UIComponent *widget, const Anchor &anchor) { mAnchor[widget] = anchor; }

		/// Retrieve the anchor data structure for a given widget
		Anchor anchor(const UIComponent *widget) const {
			auto it = mAnchor.find(widget);
			if (it == mAnchor.end())
				throw std::runtime_error("UIComponent was not registered with the grid layout!");
			return it->second;
		}
	...
	protected:
		...
		unordered_map<const UIComponent *, Anchor> mAnchor;
		int mMargin;
	};
}
