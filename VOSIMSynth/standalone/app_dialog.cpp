...
void PopulateAudioDialogs(HWND hwndDlg) {
    ...
    LRESULT iovsidx = SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IOVS, CB_FINDSTRINGEXACT, -1,
    ...
    LRESULT sigvsidx = SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_SIGVS, CB_FINDSTRINGEXACT, -1,
...
bool PopulateMidiDialogs(HWND hwndDlg) {
    if (!gMidiIn || !gMidiOut)
        ...
        LRESULT indevidx = SendDlgItemMessage(hwndDlg,IDC_COMBO_MIDI_IN_DEV,CB_FINDSTRINGEXACT, -1,
        ...
        LRESULT outdevidx = SendDlgItemMessage(hwndDlg,IDC_COMBO_MIDI_OUT_DEV,CB_FINDSTRINGEXACT, -1,
...
WDL_DLGRET PreferencesDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case IDC_COMBO_AUDIO_OUT_L:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                gState->mAudioOutChanL = (int)SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_L, CB_GETCURSEL, 0, 0) +
                                         1;
...
#ifdef _WIN32
        ClientResize(hwnd, GUI_WIDTH, GUI_HEIGHT);
    ...
    case WM_COMMAND:
        ...
        case ID_ABOUT:
            if (!gPluginInstance->HostRequestingAboutBox()) {
                ...
                MessageBox(hwnd, version, BUNDLE_NAME, MB_OK);
            }
            return 0;
        case ID_PREFERENCES:
        {
            INT_PTR ret = DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG_PREF), hwnd, PreferencesDlgProc);
