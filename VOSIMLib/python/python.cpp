...
PYBIND11_PLUGIN(pyVOSIMLib) {
        ...
        .def("tick", [](syn::Unit& self, const MatrixXdR& a_inputs)
            {
                if (a_inputs.rows() > self.numInputs())
                    throw std::runtime_error(
                        "Input buffer should have at most " + std::to_string(self.numOutputs()) + " rows.");
    ...
    py::class_<syn::Circuit, PyUnit<syn::Circuit>>(m, "Circuit", unit)
        ...
        .def("connect",
            (bool (syn::Circuit::*)(const syn::Unit&, int, const syn::Unit&, int))&syn::Circuit::connectInternal)
    ...
    affineLut.def("__init__",
                 [](syn::AffineTable& inst, const Eigen::RowVectorXd& a_table, double a_inputMin = 0,
                    double a_inputMax = 1)
                 {
    ...
    rstable.def("__init__",
               [](syn::ResampledTable& inst, const Eigen::RowVectorXd& a_table,
                  const syn::BlimpTable& a_blimp_table_online, const syn::BlimpTable& a_blimp_table_offline)
               {
                   new(&inst) syn::ResampledTable(a_table.data(), a_table.cols(), a_blimp_table_online,
                       a_blimp_table_offline);
               })
           ...
           .def("get", [](syn::ResampledTable& a_self, py::array_t<double> a_phase, double a_period)
           {
               auto closure = [&a_self, a_period](double a_phaseVec)
               {
                   return a_self.getResampled(a_phaseVec, a_period);
               };
               return py::vectorize(closure)(a_phase);
           })
           .def_property_readonly("size", [](const syn::ResampledTable& a_self) { return a_self.m_size; })
           .def_property_readonly("resampled_tables", [](const syn::ResampledTable& a_self)
           {
               return a_self.resampledTables();
           });
    ...
    m.def("blimp_online", &syn::lut_blimp_table_online, "Online blimp table.",
        pybind11::return_value_policy::reference);
    m.attr("blsaw") = syn::lut_bl_saw_table();
    m.attr("bltri") = syn::lut_bl_tri_table();
    m.attr("blsqr") = syn::lut_bl_square_table();
    m.attr("pitchtable") = syn::lut_pitch_table();
    m.attr("sintable") = syn::lut_sin_table();

    m.def("resample",
        [](const Eigen::RowVectorXd& a_input, double a_newSize, const syn::BlimpTable& a_blimpTable,
           bool a_normalize)
        {
        ...
        py::arg("input"), py::arg("period"), py::arg("blimp_table"), py::arg("normalize") = true,
