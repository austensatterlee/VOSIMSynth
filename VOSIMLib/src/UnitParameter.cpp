...
namespace syn
{
        ...
        m_parent(nullptr)
    {
        ...
        {
            if (a_optionValues.empty()) { optionValue = i; }
            else { optionValue = a_optionValues[i]; }
    ...
    string UnitParameter::getValueString() const
    {
        ...
        if (getType() != Double) { snprintf(displaytext, MAX_PARAM_STR_LEN, "%d", idx); }
        else
        {
            double displayValue = getDouble();
            // if precision is set, print out that many digits
            if (getPrecision() > 0) { snprintf(displaytext, MAX_PARAM_STR_LEN, "%.*f", m_displayPrecision, displayValue); }
