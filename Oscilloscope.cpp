...
namespace syn
{
	...
	void Oscilloscope::process()
	{
		...
		if (currInput == nullptr || currTrigger == nullptr)
			return;
			...
			if (
				...
				if (m_periodCount >= m_displayPeriods) {
					...
					for (int j = m_inputBuffer.size() - 1; j >= 0; j--) {
						if (inputBufInd < 0)
							inputBufInd = m_inputRingBuffer.size() - 1;
	...
	bool Oscilloscope::Draw(IGraphics* pGraphics)
	{
		...
		for (int j = 1; j <= bufreadlen; j += 1) {
			rms += pow(m_config->outputbuf[j - 1], 2);
		...
		sprintf(gridstr, "Displaying %d periods | Buffer size: %d (in) %d (out) | Input frequency: %.2f Hz (%.2f Hz) | Amp: %.2f | RMS: %.2f",
		        m_displayPeriods, m_BufSize, bufsize, estfreq, publishedfreq, 0.5 * (maxY - minY), rms);
		m_mutex.Leave();
