  ...
  :	IPLUG_CTOR(kNumParams, kNumPrograms, instanceInfo)
{
  ...
  mOscilloscope = new Oscilloscope(this,IRECT(10,310,800-10,600-10),100);
...
void VOSIMSynth::OnNote(uint8_t pitch, uint8_t vel) {
  ...
  mOscilloscope->setBufSize(8. / mVoiceManager.mVoiceStack.front()->mOsc[0].mStep + 1);
}
...
void VOSIMSynth::OnParamChange(int paramIdx)
{
  ...
  switch (paramIdx)
  {
    ...
    case kOsc1Vol:
        while ( n-- )
            mVoiceManager.mVoices[n].mOsc[0].mpGain.set( LERP(-60,6,GetParam( kOsc1Vol )->Value()) );
        break;
    ...
    case kOsc2Vol:
        while (n--)
            mVoiceManager.mVoices[n].mOsc[1].mpGain.set(LERP(-60, 12, GetParam(kOsc2Vol)->Value()));
        break;
    ...
    case kOsc3Vol:
        while (n--)
            mVoiceManager.mVoices[n].mOsc[2].mpGain.set(LERP(-60, 12, GetParam(kOsc3Vol)->Value()));
        break;
    
    /* Global Pitch LFO */
    case kGlobalPMF:
        while (n--)
            mVoiceManager.mVoices[n].mLFOPitch.mpPitch.set(48*GetParam(kGlobalPMF)->Value()-24);
        break;
    case kGlobalPMG:
        while (n--)
            mVoiceManager.mVoices[n].mLFOPitch.mpGain.set(LERP(-36,22,GetParam(kGlobalPMG)->Value()));
        break;
    default:
      break;
  }
