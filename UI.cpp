...
namespace syn
{
  ...
  {
    if (newsize <= m_nParams*10+10) newsize = m_nParams*10+10;
    m_size = newsize;
    mRECT.R = m_x + m_size;
    mRECT.B = m_y + m_size;
  ...
  {
    return NDPoint<2,int>(m_ports[port].L,m_ports[port].T);
  }
  ...
  bool UnitControl::showContextMenu(int x, int y)
  {
    ...
    if (selectedMenu == &m_menu)
    {
      ...
      if (itemChosen == 0)
      {
        m_unit->getParent().setSinkId(m_unit->getParent().getUnitId(m_unit));
        return true;
      }
    }
    return false;
  ...
  void UnitPanel::OnMouseUp(int x, int y, IMouseMod* pMod)
  {
    ...
    { // Right clicking on open space
      ...
      { // Create a source unit
        ...
        m_vm->getProtoInstrument()->addSource(srcunit);
        m_unitControls.push_back(new UnitControl(mPlug, srcunit, x, y));
        updateInstrument();
      }
      ...
      { // Create a non-source unit
        ...
        m_vm->getProtoInstrument()->addUnit(unit);
        m_unitControls.push_back(new UnitControl(mPlug, unit, x, y));
        updateInstrument();
      }
    }
    else if (m_isMouseDown == 2 && currSelectedUnit >= 0)
    { // Right clicking on a unit
      bool result = m_unitControls[m_lastSelectedUnit]->showContextMenu(x, y);
      if (result)
      {
        updateInstrument();
      }
    ...
    {
      m_vm->getProtoInstrument()->addConnection({currSelectedUnit, m_lastSelectedUnit, m_lastSelectedParam, SET});
      updateInstrument();
    }
    m_currAction = NONE;
    m_isMouseDown = 0;
  ...
  bool UnitPanel::Draw(IGraphics* pGraphics)
  {
    ...
    for (int i = 0; i < m_unitControls.size(); i++)
    {
      const vector<ConnectionMetadata>& connections = instr->getConnectionsTo(i);
